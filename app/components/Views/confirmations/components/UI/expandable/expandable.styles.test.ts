...
describe('expandable.styles', () => {
  ...
  it('uses elevated surface color for the bottom modal container', () => {
    const styles = styleSheet({ theme: darkThemeModel, vars: { isCompact: false } });

    expect(styles.modalContent.backgroundColor).toBe(
  ...
  it('does not apply muted border when pure black preview is off', () => {
    ...
    expect(styles.modalContent.borderWidth).toBe(0);
    expect(styles.modalContent.borderColor).toBeUndefined();
  });
  ...
  it('applies muted border in pure black dark mode', () => {
    ...
    expect(styles.modalContent.borderWidth).toBe(1);
    expect(styles.modalContent.borderColor).toBe(
